...
const { DB_CH10 } = require('./ch10_db');

function main() {
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch10.json';
  const tplFile    = process.argv[3] || path.join(__dirname, 'ch10_template.txt');
...
const samplesDir = path.dirname(jsonPath);
  ...
  function resolvePersonal(tag) {
    ...
    if (tag === 'PERSONAL.건강') return applySlots(건강블록[slots.건강관심||'기본'] || 건강블록['기본']);
    return null;
  }
  ...
  if (tag === 'BRAND.안내') {
    ...
    if (parts[0] !== 'CH10') return `[NS없음: ${parts[0]}]`;
    const section = parts[1];
    const key     = parts[2];
    const node    = DB_CH10[section];
    if (!node) return `[CH10없음: ${section}]`;
    if (typeof node === 'string') return applySlots(node);
    if (key !== undefined) {
      const val = node[key];
      if (val === undefined) return `[CH10없음: ${section}.${key}]`;
      if (typeof val === 'string') return applySlots(val);
      return `[CH10없음: ${section}.${key}(타입오류)]`;
    }
    return `[CH10없음: ${section}(키없음)]`;
  }
  ...
  const outPath = path.join(samplesDir, `${fileId}_ch10_result.txt`);
