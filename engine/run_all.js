...
// 슬롯 모드면 virtualSlotId(슬롯 경로 포함)로 호출 — generate_cover.js가 master.json 추적
try {
...
// table_template.json이 있으면 그 배치를 따르고, 없으면 기본 규칙 사용
{
  ...
  // 이미 본문에 존재하는 [[TABLE:...]] 태그를 먼저 _inserted에 등록 (템플릿 직박으로 인한 중복 방지)
  {
    const _m = _tc.match(/\[\[TABLE:([^\]]+)\]\]/g) || [];
    _m.forEach(t => {
      const name = t.replace(/^\[\[TABLE:/, '').replace(/\]\]$/, '');
      _inserted.add(name);
    });
  }
...
// ⚠️ cover.html은 generate_cover.js가 slotDir/tables/ 에 직접 쓰므로 삭제 시 보존 필요
if (isSlotMode) {
  try {
    ...
    if (fs.existsSync(srcDir)) {
      ...
      // 단, cover.html은 generate_cover.js가 직접 쓴 파일이므로 보존
      if (fs.existsSync(dstDir)) {
        for (const f of fs.readdirSync(dstDir)) {
          if (f.endsWith('.html') && f !== 'cover.html') fs.unlinkSync(path.join(dstDir, f));
        }
      } else {
...
// ── 중간 파일 정리 (ch*_result.txt + master_preprocessed) ──────
// CHECK 45 (필수데이터누락) 검증을 위해 ch*.json은 유지
// 정리 대상: ch*_result.txt (rendered text 임시), mockcha*.*, master_preprocessed.json
// 유지 대상: ch*.json (검증·재집필 시 캐시), master.json, all_*_result.txt
{
  let cleanedCount = 0;
  const queueFiles = fs.readdirSync(targetDir);
  for (const f of queueFiles) {
    if (!f.startsWith(fileId + '_')) continue;
    // 삭제 대상: ch*_result.txt (chapter 렌더링 임시), mockcha 모든 파일
    const isChResult = /_(ch)[^.]*_result\.txt$/.test(f);
    const isMockcha  = /_mockcha/.test(f) && (f.endsWith('.json') || f.endsWith('_result.txt'));
    const isPreprocessed = f === `${fileId}_master_preprocessed.json`;
    if (isChResult || isMockcha || isPreprocessed) {
  ...
  if (isSlotMode) {
    ...
    // (단, ch*.json은 유지 — CHECK 45 검증용)
    try {
      for (const f of fs.readdirSync(samplesDir)) {
        if (!f.startsWith(fileId + '_')) continue;
        // queue 루트에 ch*.json이 있다면 잘못된 위치이므로 정리
  ...
  if (cleanedCount > 0)
    console.log(`  🧹 중간 파일 ${cleanedCount}개 정리됨 (ch*.json은 검증용으로 유지)`);
}
