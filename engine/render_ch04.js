...
function _injectSinsal(M) {
  ...
  if (!M._신강약설명) {
    M._신강약설명 = (M.신강약||'').includes('신강(')
      ...
      : '신약(身弱) 구조입니다. 유연하고 협력에 강해요. 인성(인성)·비겁(비겁) 대운(대운)에 전환점이 옵니다.';
  }
  // 신강약 정규화
  if (M.신강약) {
    const r = M.신강약;
...
const { DB_CH04 } = require('./ch04_db');


function _getAnimalEmoji(animal) {
  const map = { '쥐':'🐭','소':'🐮','호랑이':'🐯','토끼':'🐰','용':'🐲','뱀':'🐍','말':'🐴','양':'🐑','원숭이':'🐵','닭':'🐔','개':'🐶','돼지':'🐷' };
  for (const [k,v] of Object.entries(map)) if (animal.includes(k)) return v;
  return '✨';
}
...
function main() {
  ...
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch04.json';
  const tplFile    = process.argv[3] || path.join(__dirname, 'ch04_template.txt');
...
const samplesDir = path.dirname(jsonPath);
  ...
  const outPath = path.join(samplesDir, `${fileId}_ch04_result.txt`);
