...
const { DB_CH17: DB_EPILOGUE } = require('./ch17_db');

function main() {
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch17.json';
  const tplFile    = process.argv[3] || path.join(__dirname, 'ch17_template.txt');
...
const samplesDir = path.dirname(jsonPath);
  ...
  // 천간합화오행 없을 때 대체값
  if (!slots['천간합화오행'] || slots['천간합화오행'] === '없음') {
    slots['천간합화오행'] = '해당없음';
  }
  ...
  if (tag === 'BRAND.안내') {
    ...
    if (parts[0] === 'CH17') {
      const { DB_CH17 } = require('./ch17_db');
      const node17 = DB_CH17[parts[1]];
      if (!node17) return `[CH17없음: ${parts[1]}]`;
      return typeof node17 === 'string' ? applySlots(node17) : '';
    }
    if (parts[0] === 'APPENDIX') {
      const { DB_CH18 } = require('./ch18_db');
      const node18 = DB_CH18[parts[1]];
      if (!node18) return `[APPENDIX없음: ${parts[1]}]`;
      return typeof node18 === 'string' ? applySlots(node18) : '';
    }
    if (parts[0] !== 'EPILOGUE') return `[NS없음: ${parts[0]}]`;
    const section = parts[1];
    const key     = parts[2];
    const node    = DB_EPILOGUE[section];
    if (!node) return `[EPILOGUE없음: ${section}]`;
    if (typeof node === 'string') return applySlots(node);
    if (key !== undefined) {
      const val = node[key];
      if (val === undefined) return `[EPILOGUE없음: ${section}.${key}]`;
      if (typeof val === 'string') return applySlots(val);
    }
    return `[EPILOGUE없음: ${section}(키없음)]`;
  }
  ...
  // 1. 없음 → 원국 부재 치환 (먼저)
  // '없음' 값 슬롯 후처리
  tpl = tpl.replace(/\{\{배우자성종류\}\}/g, '원국 부재');
  tpl = tpl.replace(/\{\{배우자성위치\}\}/g, '없음');
  tpl = tpl.replace(/\{\{지지파해목록\}\}/g, '없음');
  tpl = tpl.replace(/\{\{[^}]+\}\}/g, '');  // 나머지 미치환 슬롯 제거
  tpl = tpl.replace(/없음이에요/g, '없는 구조이에요');
  tpl = tpl.replace(/천간합\(天干合\): 없음/g, '천간합(天干合): 원국 부재');
  tpl = tpl.replace(/지지합충파해.地支.: 없음 \/ 없음/g, '지지합충파해(地支): 원국 부재');
  tpl = tpl.replace(/지지합충파해.地支.: 없음 \//g, '지지합충파해(地支): ');
  tpl = tpl.replace(/관성.官星.: 없음 .없음./g, '관성(官星): 원국 부재');
  tpl = tpl.replace(/관살.官殺.: 없음 .없음, 없음./g, '관살(官殺): 원국 부재');
  tpl = tpl.replace(/\{\{(천간합목록|지지합목록|관성종류|관성위치|관살종류|관살위치|관살강도|자녀성종류|태극귀인위치|문창귀인위치|천복귀인위치|재성종류|재성위치|부모성위치|천을귀인위치|인성위치)\}\}/g, '원국 부재');
  tpl = tpl.replace(/자녀성: N/g, '자녀성: 원국 부재');
  tpl = tpl.replace(/자녀성: Y/g, '자녀성: 있음');

  // 2. 천간합 없음 케이스 》 "원국 부재" 확정 이후에 설명 교체
  tpl = tpl.replace(
    /천간합\(天干合\): 원국 부재\n→[^\n]*/,
    '천간합(天干合): 원국 부재\n→ 천간합이 없는 구조이에요. 천간의 오행이 합으로 변화하지 않고 원래 특성을 그대로 유지해요. 기질이 일관되고 꾸준한 것이 강점이에요.'
  );

  // 3. 기타 후처리
  tpl = tpl.replace(/이 대운은 앞으로 0년이 남아 있어요\./g, '이 대운은 올해 새로운 대운으로 전환되는 바로 그 시점이에요.');
  tpl = tpl.replace(/대운 대운으로/g, '대운으로');
  // 귀인 위치가 '없음'인 줄 제거
  tpl = tpl.replace(/^.*위치: 없음\s*\n/gm, '');
  // 귀인 위치 미치환 슬롯 제거 (없음 값이 ch17에 전달 안된 경우 방어)
  tpl = tpl.replace(/^.*\{\{(천을귀인위치|태극귀인위치|문창귀인위치|천복귀인위치|천덕귀인위치|월덕귀인위치)\}\}\s*\n/gm, '');
  // 위치값이 빈칸인 줄 제거 (슬롯 미치환 방어)
  tpl = tpl.replace(/^.*(天乙貴人|文昌貴人|天福貴人|太極貴人|天德貴人|月德貴人).*위치:\s*\n/gm, '');
  tpl = tpl.replace(/\n{4,}/g, '\n\n\n').trim() + '\n';

  const fileId  = M.id || M.이름;
  const outPath = path.join(samplesDir, `${fileId}_ch17_result.txt`);
