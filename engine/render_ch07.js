...
function main() {
  ...
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch07.json';
  const tplFile    = process.argv[3] || path.join(__dirname, 'ch07_template.txt');
...
const samplesDir = path.dirname(jsonPath);
  ...
  if (tag === 'BRAND.안내') {
    ...
    if (parts[0] !== 'CH07') return `[NS없음: ${parts[0]}]`;
    if (tag === 'CH07.지장간표' || parts[1] === '지장간표') return '';
    const section = parts[1];
    const key     = parts[2];
    const node    = DB_CH07[section];
    if (!node) return `[CH07없음: ${section}]`;
  ...
  const outPath = path.join(samplesDir, `${fileId}_ch07_result.txt`);
