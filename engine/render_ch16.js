...
const { DB_CH16 } = require('./ch16_db');

function main() {
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch16.json';
  const tplFile    = process.argv[3] || path.join(__dirname, 'ch16_template.txt');
...
const samplesDir = path.dirname(jsonPath);
  ...
  if (tag === 'BRAND.안내') {
    ...
    if (tag.startsWith('PERSONAL.')) {
      const r = resolvePersonal(tag); if (r !== null) return r;
    }
    const parts = tag.split('.');
    if (parts[0] !== 'CH16') return `[NS없음: ${parts[0]}]`;
    const section = parts[1];
    const key     = parts[2];
    const node    = DB_CH16[section];
    if (!node) return `[CH16없음: ${section}]`;
  ...
  // 조사 교정: 정관를→정관을, 없음를→없음을, 편관를→편관을
  tpl = tpl.replace(/정관를/g, '정관을');
  tpl = tpl.replace(/없음를/g, '없음을');
  tpl = tpl.replace(/편관를/g, '편관을');
  tpl = tpl.replace(/편관·정관를/g, '편관·정관을');
  // 형충파해 목록 뒤 조사 오류: "...이 있어요" 이중 조사
  tpl = tpl.replace(/(파\([破]\)[^\n]+|해\([害]\)[^\n]+|형\([刑]\)[^\n]+|충\([冲]\)[^\n]+)이 있어요/g, '$1이 있어요');
  // 마무리 요약 "없음 (없음)" → "없음 (원국 부재)"
  tpl = tpl.replace(/없음 \(없음\)/g, '없음 (원국 부재)');
  tpl = tpl.replace(/\n{4,}/g, '\n\n\n').trim() + '\n';

  const fileId  = M.id || M.이름;
  const outPath = path.join(samplesDir, `${fileId}_ch16_result.txt`);
