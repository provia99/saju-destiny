...
function generate(slotId){
  ...
  if(!fs.existsSync(ch03File)){console.error('❌ 없음:',ch03File);process.exit(1);}
  ...
  // 오행 점수
  const SC={木:+(d4['목점수']||d1['목점수']||mp['목점수']||0),火:+(d4['화점수']||d1['화점수']||mp['화점수']||0),土:+(d4['토점수']||d1['토점수']||mp['토점수']||0),金:+(d4['금점수']||d1['금점수']||mp['금점수']||0),水:+(d4['수점수']||d1['수점수']||mp['수점수']||0)};
