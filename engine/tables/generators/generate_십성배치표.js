...
function generate(slotId) {
  ...
  try {
    ...
    if (masterPath) {
      ...
      const r = 전체사주계산({ 이름:m.이름, 성별:m.성별, 년:m.생년, 월:m.생월, 일:m.생일,
        시간:m.생시||'모름', 음력입력:!!m.음력입력, 윤달:!!m.윤달 });
  ...
  const baechiMap = parseSipseongBaechi(baechiStr);

  // 있는/없는 십성 목록
  const existList   = (d5['있는십성목록'] || '').split('·').map(s=>s.trim()).filter(Boolean);
  const missingList = (d5['없는십성목록'] || '').split('·').map(s=>s.trim()).filter(Boolean);
  const existSet    = new Set(existList.map(s => s.replace('일원(日元)','일원(나)')));

  // 계열 정보
  const existGyeyeol  = (d5['있는계열목록'] || '').split('·').map(s=>s.trim()).filter(Boolean);
  const missingGyeyeol = (d5['없는계열목록'] || '').split('·').map(s=>s.trim()).filter(Boolean);
  const maxGyeyeol    = d5['최다십성계열'] || '';
  const maxGyeyeolCnt = d5['최다십성계열수'] || '';
...
.sipseong-table th { font-size:7pt; font-weight:700; color:#555; background:#f8f8f8; padding:1px 4px; border:1px solid #333; text-align:center; }
...
.sipseong-table td { border:1px solid #333; padding:2px 4px; vertical-align:middle; font-size:7pt; }
...
<div class="card">
  ...
  <table class="sipseong-table">
    <thead>
      <tr>
        <th style="width:14mm;">계열</th>
        <th style="width:22mm;">십성</th>
        <th style="width:14mm;">유무</th>
        <th style="width:30mm;">위치</th>
        <th>의미·기질</th>
        <th style="width:35mm;">삶의 영역</th>
      </tr>
    </thead>
