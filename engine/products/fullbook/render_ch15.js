...
const { DB_CH15 } = require('./ch15_db');

function main() {
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch15.json';
  const tplFile    = process.argv[3] || path.join(__dirname, 'ch15_template.txt');
...
const samplesDir = path.dirname(jsonPath);
  ...
  if (tag === 'BRAND.안내') {
    ...
    if (tag.startsWith('TABLE:')) return `[[${tag}]]`; // TABLE 태그는 후처리로 통과
    if (parts[0] !== 'CH15') return `[NS없음: ${parts[0]}]`;
    const section = parts[1];
    const key     = parts[2];
    const node    = DB_CH15[section];
    if (!node) return `[CH15없음: ${section}]`;
    if (typeof node === 'string') return applySlots(node);
    if (key !== undefined) {
      const val = node[key];
      if (val === undefined) return `[CH15없음: ${section}.${key}]`;
      if (typeof val === 'string') return applySlots(val);
      return `[CH15없음: ${section}.${key}(타입오류)]`;
    }
    return `[CH15없음: ${section}(키없음)]`;
  }
  ...
  const outPath = path.join(samplesDir, `${fileId}_ch15_result.txt`);
