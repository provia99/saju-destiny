...
const { 브랜드블록, injectCh08Fields } = require('./ch_personal_db');
const { DB_CH18: DB_APPENDIX } = require('./ch18_db');




function main() {
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch18.json';
...
const samplesDir = path.dirname(jsonPath);
  ...
  let result = applySlots(전체);

  // 후처리
  // 이중괄호 방지: (丙午(병오)년) → 丙午(병오)년
  // 이중괄호 패턴 제거: (간지(음독)년) → 간지(음독)년
  result = result.replace(/\(([^()]+\([가-힣]+\))년\)/g, '$1년');
  // 구성 번호 재정렬 (비연속 번호 → 연속 번호)
  // 부록 번호 재정렬 불필요 (DB에서 정확히 지정)
  result = result.replace(/\n{4,}/g, '\n\n\n').trim() + '\n';

  const fileId  = M.id || M.이름;
  const outPath = path.join(samplesDir, `${fileId}_ch18_result.txt`);
  result = applySlots(result);

  fs.writeFileSync(outPath, result, 'utf8');
  console.log(`✅ 렌더링 완료: queue/${path.basename(outPath)}`);
  console.log(`📄 글자수: ${[...result].length}자`);
}
main();
