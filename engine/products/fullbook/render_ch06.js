...
const { 브랜드블록, 결혼블록, 결혼블록_ch06, 자녀블록, 고민강조블록, 고민강조블록_짧음, 형제블록, 부모블록, 건강블록, injectCh08Fields } = require('./ch_personal_db');

const fs   = require('fs');
const path = require('path');
const { DB_CH06 } = require('./ch06_db');
...
function main() {
  ...
  function resolvePersonal(tag, slots) {
    ...
    if (tag === 'PERSONAL.결혼') {
      const 상태 = M.결혼상태 || '미혼';
      const blk = 결혼블록_ch06[상태] || 결혼블록_ch06['미혼'] || 결혼블록[상태] || 결혼블록['미혼'];
      return applyS(blk);
    }
    if (tag === 'PERSONAL.자녀') {
  ...
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch06.json';
  const tplFile    = process.argv[3] || path.join(__dirname, 'ch06_template.txt');
...
const samplesDir = path.dirname(jsonPath);
  ...
  if (tag === 'BRAND.안내') {
    ...
    if (parts[0] !== 'CH06') return `[NS없음: ${parts[0]}]`;
    if (tag === 'CH06.용신가이드카드' || parts[1] === '용신가이드카드') return '';
    if (tag === 'CH06.4신요약표' || parts[1] === '4신요약표') return '';
    if (parts[1] === '용신_정의') return '';
    const section = parts[1];
    const key     = parts[2];
    const node    = DB_CH06[section];
    if (!node) return `[CH06없음: ${section}]`;
  ...
  const outPath = path.join(samplesDir, `${fileId}_ch06_result.txt`);
