...
const { 브랜드블록, 결혼블록, 결혼블록_ch01, 자녀블록, 고민강조블록, 고민강조블록_짧음, 형제블록, 부모블록, 건강블록, injectCh08Fields } = require('./ch_personal_db');

const fs   = require('fs');
const path = require('path');
const { DB_CH01 } = require('./ch01_db');
...
function resolveBlock(key, M) {
  ...
  injectCh08Fields(M, samplesDir, slots);
    ...
    if (key === 'PERSONAL.결혼') {
      const blk = 결혼블록_ch01[M.결혼상태||'미혼'] || 결혼블록_ch01['미혼'] || 결혼블록[M.결혼상태||'미혼'] || 결혼블록['미혼'];
      return applyS(blk);
    }
  ...
  if (parts[0] !== 'CH01') return `[NS없음: ${parts[0]}]`;
  const k = parts[1];
  const sub = parts[2];
  if (!DB_CH01[k]) return `[CH01없음: ${k}]`;
  if (typeof DB_CH01[k] === 'object' && sub) {
    const val = DB_CH01[k][sub];
    if (!val) return `[CH01없음: ${k}.${sub}]`;
    return String(val);
  }
  if (typeof DB_CH01[k] === 'object') return `[CH01없음: ${k}(서브키필요)]`;
  return String(DB_CH01[k]);
}
...
function main() {
  ...
  function resolvePersonal(tag, slots) {
    ...
    if (tag === 'PERSONAL.결혼') {
      const 상태 = M.결혼상태 || '미혼';
      const blk = 결혼블록_ch01[상태] || 결혼블록_ch01['미혼'] || 결혼블록[상태] || 결혼블록['미혼'];
      return applyS(blk);
    }
    if (tag === 'PERSONAL.자녀') {
    ...
    if (tag === 'PERSONAL.고민') {
      ...
      return applyS(고민강조블록[분야] || 고민강조블록['종합']);
    }
    if (tag === 'PERSONAL.형제') {
  ...
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch01.json';
  const tmplFile   = process.argv[3] || path.join(__dirname, 'ch01_template.txt');
...
const samplesDir = path.dirname(jsonPath);
  ...
  const outPath = path.join(samplesDir, `${fileId}_ch01_result.txt`);
