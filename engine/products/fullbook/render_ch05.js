...
const { 브랜드블록, 결혼블록, 결혼블록_ch05, 자녀블록, 고민강조블록, 고민강조블록_짧음, 형제블록, 부모블록, 건강블록, injectCh08Fields } = require('./ch_personal_db');

const fs   = require('fs');
const path = require('path');
const { DB_CH05 } = require('./ch05_db');
...
function main() {
  ...
  function resolvePersonal(tag, slots) {
    ...
    if (tag === 'PERSONAL.결혼') {
      const 상태 = M.결혼상태 || '미혼';
      const blk = 결혼블록_ch05[상태] || 결혼블록_ch05['미혼'] || 결혼블록[상태] || 결혼블록['미혼'];
      return applyS(blk);
    }
    if (tag === 'PERSONAL.자녀') {
  ...
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch05.json';
  const tplFile    = process.argv[3] || path.join(__dirname, 'ch05_template.txt');
...
const samplesDir = path.dirname(jsonPath);
  ...
  if (tag === 'BRAND.안내') {
    ...
    if (parts[0] !== 'CH05') return `[NS없음: ${parts[0]}]`;
    if (tag === 'CH05.십성배치표' || parts[1] === '십성배치표') return '';
    if (tag === 'CH05.십성균형표' || parts[1] === '십성균형표') return '';
    const section = parts[1];
    const key     = parts[2];

    // 특수 동적 블록 》 DB에 키가 없으므로 먼저 처리
    if (section === '있는십성_해석') {
      const 십성목록 = M.있는십성목록.split('·').filter(s => s && s !== '없음');
      return 십성목록.map(십성 => {
        const blk = DB_CH05['십성_해석'][십성.trim()];
        return blk ? applySlots(blk) : `[CH05없음: 십성_해석.${십성}]`;
      }).join('\n\n');
    }

    if (section === '없는십성_미리보기') {
      if (typeof DB_CH05.없는십성_미리보기 === 'function') {
        return applySlots(DB_CH05.없는십성_미리보기(M));
      }
      return '';
    }

    if (section === '없는계열_해석_전체') {
      const 없는계열 = M.없는계열목록.split('·').filter(s => s && s !== '없음');
      if (없는계열.length === 0) return '';
      return 없는계열.map(계열 => {
        const blk = DB_CH05['없는계열_해석'][계열.trim()];
        return blk ? applySlots(blk) : `[CH05없음: 없는계열_해석.${계열}]`;
      }).join('\n\n');
    }

    const node = DB_CH05[section];
    if (!node) return `[CH05없음: ${section}]`;
    // 동적 함수형 블록: (M) => string 형태
    if (typeof node === 'function') return applySlots(node(M));
  ...
  // ── 없는계열 전부 있는 경우: 빈 헤더 아래 대체 문장 삽입 ──
  if (!M.없는계열목록 || M.없는계열목록 === '없음') {
    const 대체문 = M.이름 + ' 님의 사주에는 비겁(比劫)·식상(食傷)·재성(財星)·관성(官星)·인성(印星) 다섯 계열이 모두 갖추어져 있어요.\n\n' +
      '한 가지 별도 빠짐없이 자리를 지키는 구조이에요. 결핍보다 균형을 타고난 사주이에요. 어느 한 방향으로 치우치지 않고 상황에 따라 다양한 기운을 활용할 수 있다는 뜻이기도 해요. ' +
      (M.선생님이름 || '반야선생') + '이 봐온 분들 중에 이렇게 다섯 계열이 고루 있는 분들은 상황 적응력이 뛰어나고, 특정 분야에 한정되지 않는 넓은 그릇을 가진 경우가 많았어요.\n\n' +
      '→ ' + M.이름 + ' 님의 경우:\n' +
      '다섯 계열이 모두 있는 균형 구조이에요. 특정 한 별이 압도적으로 강한 것보다 다양한 상황에 유연하게 대응할 수 있는 강점이에요. 가장 강한 계열 ' + (M.최다십성계열 || '') + '의 방향으로 에너지를 집중할 때 가장 자연스럽게 빛납니다.';
    tpl = tpl.replace(
      /(✺ 제4절\. 없는 별의 의미[ \t]*\n)((?:[ \t]*\n)*)/,
      (_, hdr, blanks) => hdr + '\n' + 대체문 + '\n\n'
    );
  ...
  const outPath = path.join(samplesDir, `${fileId}_ch05_result.txt`);
