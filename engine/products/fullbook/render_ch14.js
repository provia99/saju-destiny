...
const { DB_CH14 } = require('./ch14_db');

function main() {
  const jsonArg    = process.argv[2] || 'choi_wonsuk_ch14.json';
  const tplFile    = process.argv[3] || path.join(__dirname, 'ch14_template.txt');
  const samplesDir = path.join(__dirname, 'queue');
  const jsonPath   = path.isAbsolute(jsonArg) ? jsonArg : path.join(samplesDir, jsonArg);
  ...
  // 슬롯 치환 》 [[블록]] 태그 내부는 보호
  // [[...]] 태그를 임시 플레이스홀더로 대체 후 슬롯치환, 복원
  const _blockTokens = [];
  tpl = tpl.replace(/\[\[[^\]]+\]\]/g, m => {
    const token = `__BLOCK_${_blockTokens.length}__`;
    _blockTokens.push(m);
    return token;
  });
  for (const [k, v] of Object.entries(slots)) tpl = tpl.replaceAll(`{{${k}}}`, v);
  // 복원
  tpl = tpl.replace(/__BLOCK_(\d+)__/g, (_, i) => _blockTokens[parseInt(i)]);

  // template 레벨 <<IF>> 처리 (블록치환 전 》 불필요한 블록 제거)
  let _tifPrev;
  do {
    _tifPrev = tpl;
  ...
  // <<IF>> 처리는 [[블록]] 치환 이후에 수행 (아래로 이동) 》 이 주석은 제거
  ...
  function resolvePersonal(tag) {
    if (tag === 'PERSONAL.결혼') return applySlots(결혼블록[slots.결혼상태||'미혼'] || 결혼블록['미혼']);
  ...
  if (tag === 'BRAND.안내') {
    ...
    if (parts[0] !== 'CH14') return `[NS없음: ${parts[0]}]`;
    const section = parts[1];
    const key     = parts[2];
    const node    = DB_CH14[section];
    if (!node) return `[CH14없음: ${section}]`;
    if (typeof node === 'string') return applySlots(node);
    if (key !== undefined) {
      const val = node[key];
      // 키가 없으면 빈 문자열 반환 (남자가 여자 블록 접근하면 빈 문자열)
      if (val === undefined) return '';
      if (typeof val === 'string') return applySlots(val);
      return `[CH14없음: ${section}.${key}(타입오류)]`;
    }
    return `[CH14없음: ${section}(키없음)]`;
  }
  ...
  // 자녀인연 없음은 원국부재로, 있는 경우엔 (원국) 제거
  tpl = tpl.replace(/^자녀 인연: 없음.*$/mg, '자녀 인연: 원국 부재');
  tpl = tpl.replace(/ \(원국\)/g, '');
  // 이미 원국부재 처리된 것 복구
  tpl = tpl.replace(/^자녀 인연: 원국 부재$/mg, '자녀 인연: 원국 부재');
  // 있는 자녀성에서 (원국) 제거

  tpl = tpl.replace(/없음 \(없음\)/g, '원국 부재');
  // 이중괄호 정리: (辰(진)) → 辰(진)
  tpl = tpl.replace(/\(([^()]+\([가-힣]+\))\)/g, '$1');
  tpl = tpl.replace(/\n{4,}/g, '\n\n\n').trim() + '\n';

  const fileId  = M.id || M.이름;
  const outPath = path.join(samplesDir, `${fileId}_ch14_result.txt`);
