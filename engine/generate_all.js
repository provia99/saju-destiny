...
const PRODUCT_TABLES = {
  yearly_fortune: ['cover','명식표','인적사항표','연간운세요약표','운세달력_표지','운세달력_01월','운세달력_02월','운세달력_03월','운세달력_04월','운세달력_05월','운세달력_06월','운세달력_07월','운세달력_08월','운세달력_09월','운세달력_10월','운세달력_11월','운세달력_12월'],
  ...
  saju_summary: ['cover','명식표','인적사항표','오행점수표','4신요약표','인간관계표','운세달력_표지','운세달력_01월','운세달력_02월','운세달력_03월','운세달력_04월','운세달력_05월','운세달력_06월','운세달력_07월','운세달력_08월','운세달력_09월','운세달력_10월','운세달력_11월','운세달력_12월'],
};
...
// 매칭 방식: g.file에서 'generate_<표명>.js' 추출 → allowed 셋과 비교
//          또는 g.desc가 allowed로 시작/포함하면 통과
//          (옛 방식: g.desc.replace(/\d+$/) — '4신요약표1+2' → '4신요약표1+'로 매칭 실패)
...
if (productType !== 'saju_full' && productType !== 'custom' && PRODUCT_TABLES[productType]) {
  ...
  filteredGenerators = GENERATORS.filter(g => {
    // 1) file 이름에서 표명 추출 (가장 정확)
    const fileMatch = g.file.match(/^generate_(.+?)\.js$/);
    const fromFile = fileMatch ? fileMatch[1] : '';
    if (fromFile && allowed.has(fromFile)) return true;
    // 2) desc 직접 매칭
    if (allowed.has(g.desc)) return true;
    // 3) desc 앞부분이 allowed 항목과 일치 (예: '4신요약표1+2' → '4신요약표')
    for (const a of allowed) {
      if (g.desc.startsWith(a)) return true;
    }
    return false;
  });
